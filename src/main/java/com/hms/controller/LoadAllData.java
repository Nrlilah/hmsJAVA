		...
		try {
			...
			if (rs.next()) {
				...
				PreparedStatement pst9 = con
						.prepareStatement("SELECT * FROM patient WHERE status != 0 AND YEAR(appointmentDate) = ? "
				...
				PreparedStatement pst10 = con
						.prepareStatement("SELECT * FROM patient WHERE appointmentDate >= CURDATE() "
